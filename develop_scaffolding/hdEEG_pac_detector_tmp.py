...
def main():
    ...
    def modified_analyze_pac(event_type, pair_with_spindles=False):
        ...
        if args.sw_method or args.spindle_method:
         ...
         # HERE MODIFY: Add debug print statements
        print(f"Running PAC analysis with parameters:")
        print(f"  event_type: {event_type}")
        print(f"  pair_with_spindles: {pair_with_spindles}")
        print(f"  output_dir: {params['out_dir']}")
        try:
            result = pac_processor.analyze_pac(**params)
            return result, method_dir
        except Exception as e:
    ...
    if not args.sw_method or not args.spindle_method:
        print("\nERROR: Both --sw_method and --spindle_method must be specified")
        print("This modified version only supports slow wave-spindle coupling analysis")
        return
    ...
    try:
        ...
        if results['sw_spindle'] is None:
            print("ERROR: PAC analysis returned None. Check for errors above.")
            return
            
        print(f"PAC analysis completed successfully")
        print(f"Results: {results['sw_spindle']}")
    except Exception as e:
        print(f"ERROR during PAC analysis: {e}")
        import traceback
        traceback.print_exc()
        return
    ...
    if 'sw_spindle' in results and results['sw_spindle'] is not None:
        try:
            sw_spindle_csv = os.path.join(output_dirs['sw_spindle'], "sw_spindle_coupling_pac_summary.csv")
            print(f"Exporting to: {sw_spindle_csv}")
            ...
            if export_result:
                ...
                print("WARNING: Export returned None")
        except Exception as e:
            print(f"ERROR during export: {e}")
            import traceback
            traceback.print_exc()
    else:
        print("No sw_spindle results to export")
    ...
    do_comodulogram = False  # Set to True to generate comodulograms
