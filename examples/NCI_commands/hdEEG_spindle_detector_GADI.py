...
def main():
    ...
    out_dir = os.path.join(subj_dir, "wonambi", "spindle_results")
    ...
    # path separator into the legacy CSV names below, pointing them at a
    # directory that does not exist. Escape for filenames only; the database
    # always gets the unescaped test_method.
    ...
    def report_density():
        """Log per-channel spindle density derived from the database.
        ...
        if len(df) == 0:
            logger.warning(
                "No spindle rows in the database for this scope, so there is "
                "no density to report. The coverage check below says whether "
                "that is an empty night or a lost run.")
            return
        logger.info("Spindle density (events per minute of artefact-free "
    ...
    if not args.legacy_json:
        ...
        # and a detection_runs provenance row. There is no file round-trip to
        # perform. A flat CSV can be produced on demand from the database with
        # turtlewave_hdEEG.export_events_to_csv.
    ...
    params_csv = os.path.join(out_dir, f"spindle_parameters_{test_method_str}_{freq_range}_{stages_str}.csv")
    dens_csv   = os.path.join(out_dir, f"spindle_density_{test_method_str}_{freq_range}_{stages_str}.csv")
